#!/usr/bin/env python3
"""Stronger anti-corr arm: gap FE + uncertainty/error weights vs max3."""
...
def main() -> int:
    ...
    med = float(np.median(max3))
    unc = 1.0 - np.abs(max3 - med) / (np.abs(max3 - med).max() + 1e-6)
    ...
    for seed in seeds:
        ...
        for fold, (tr, va) in enumerate(StratifiedKFold(5, shuffle=True, random_state=seed).split(features, y)):
            ...
            print(f"gapw seed={seed} fold={fold} auc={roc_auc_score(y[va], oof[va]):.5f}", flush=True)
        print(f"gapw seed={seed} OOF={roc_auc_score(y, oof):.6f}", flush=True)
        oofs.append(oof)
        tests.append(pte)
    ...
    # also try just max(max3, oof)
    fused2 = nested_select_rule(y, [max3, oof])
    print("max3×gapw", fused2["nested_oof_auc"], fused2["selected_rule"])
    best = fused if fused["nested_oof_auc"] >= fused2["nested_oof_auc"] else fused2
    if best is fused:
        tp = apply_rule(best["selected_rule"], [fr["test_gap"], fr["test_gap_bag"], fr["test_plus"], te])
    else:
        tmax = np.maximum.reduce([fr["test_gap"], fr["test_gap_bag"], fr["test_plus"]])
        tp = apply_rule(best["selected_rule"], [tmax, te])

    out = Path("artifacts/b6pro_gapw")
    out.mkdir(exist_ok=True)
    np.savez_compressed(out / "predictions.npz", y=y, oof=best["nested_oof"], test=tp, oof_gapw=oof, test_gapw=te)
    build_submission(test, sample, tp, out / "submission_b6pro.csv")
    metrics = {
        "experiment_id": "b6pro_gapw",
        "oof_auc": float(roc_auc_score(y, oof)),
        "corr_max3": float(np.corrcoef(oof, max3)[0, 1]),
        "corr_gap": float(np.corrcoef(oof, b7["gap"])[0, 1]),
        "nested_oof_auc": best["nested_oof_auc"],
        "selected_rule": best["selected_rule"],
        "four_arm_nested": fused["nested_oof_auc"],
        "pair_nested": fused2["nested_oof_auc"],
        "full_data_scores": best.get("full_data_scores", fused["full_data_scores"]),
